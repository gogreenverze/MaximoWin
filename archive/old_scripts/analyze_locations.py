#!/usr/bin/env python3
"""
Script to analyze the MXAPILOCATIONS endpoint structure.
This script will:
1. Load API key from .env file
2. Fetch data from MXAPILOCATIONS endpoint with status="OPERATING"
...
logger = logging.getLogger('analyze_locations')
...
def fetch_locations_data(limit=50):
    """
    ...
    endpoints = [
        f"{BASE_URL}/api/os/mxapilocations",  # Standard REST API format
        f"{BASE_URL}/oslc/os/mxapilocations",  # OSLC format
        f"{BASE_URL}/api/os/MXAPILOCATIONS"   # Uppercase format
    ]
    ...
    query_params_options = [
        ...
        },
        {
            "lean": "0",  # Get full response
            "oslc.pageSize": str(limit),
            "oslc.select": "location,description,status,siteid,orgid,type,glaccount,changeby,changedate,spi:location,spi:description,spi:status,spi:siteid,spi:orgid,spi:type,spi:glaccount",
            "oslc.where": "spi:status=\"OPERATING\""  # Alternative filter syntax
        },
        {
            "_maxItems": str(limit),
            "oslc.select": "*",  # Request all fields
            ...
            for headers in header_options:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"Successfully fetched locations collection. Status code: {response.status_code}")

                        # Save the raw response for debugging
                        with open('locations_raw_response.json', 'w') as f:
    ...
    if not collection_data:
        logger.error("Failed to fetch locations collection")
        return None
    ...
    if not found_member_key:
        ...
        # Check if we have typical locations fields
        location_fields = ['location', 'description', 'siteid', 'orgid', 'status', 'spi:location', 'spi:siteid']
        has_location_fields = any(field in collection_data for field in location_fields)

        if has_location_fields:
            logger.info("Response appears to contain location data directly")
            # Wrap the data in a member array for consistent processing
            collection_data = {'member': [collection_data]}
            found_member_key = 'member'
        else:
            logger.error("Response does not contain location data")
            return None
    ...
    sample_location = collection_data['member'][0] if collection_data['member'] else {}
    field_count = len(sample_location.keys())
    ...
    if field_count <= 1:
        ...
        try:
            ...
            detailed_url = f"{BASE_URL}/api/os/mxapilocations"
            detailed_params = {
                "oslc.select": "*",
                "oslc.pageSize": str(limit),
                "lean": "0",
    ...
    if collection_data['member']:
        ...
        logger.info(f"Location data has these fields: {', '.join(fields)}")

    logger.info(f"Using collection data with {len(collection_data['member'])} locations")
...
def suggest_primary_key(data, field_summary):
    ...
    priority_fields = ['location', 'locationid', 'id', 'locationsid']
...
def suggest_sqlite_schema(field_summary, primary_keys):
    ...
    common_query_fields = ['status', 'siteid', 'orgid', 'type', 'description']
...
def main():
    """Main function to fetch and analyze locations data."""
    # Fetch locations data
    locations_data = fetch_locations_data(limit=50)  # Fetch up to 50 records for better analysis

    if not locations_data:
        logger.error("Failed to fetch locations data")
        return
    ...
    logger.info(f"Fetched {len(locations_data.get('member', []))} location records")

    # Analyze field types
    field_summary = analyze_field_types(locations_data)
    ...
    potential_keys = suggest_primary_key(locations_data, field_summary)

    # Print analysis results
    print("\n=== LOCATIONS ENDPOINT ANALYSIS ===\n")
    print(f"Total records analyzed: {len(locations_data.get('member', []))}")
    ...
    # Save schema to file
    with open('locations_schema.sql', 'w') as f:
        f.write(sqlite_schema)

    logger.info("Analysis complete. Results saved to locations_data.json and locations_schema.sql")
