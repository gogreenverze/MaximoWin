#!/usr/bin/env python3
"""
Script to analyze the MXAPIPERUSER endpoint structure.
This script will:
1. Load API key from .env file
2. Fetch data from MXAPIPERUSER endpoint with status="ACTIVE"
...
logger = logging.getLogger('analyze_peruser')
...
def fetch_peruser_data(limit=50):
    """
    ...
    endpoints = [
        f"{BASE_URL}/api/os/mxapiperuser",  # Standard REST API format
        f"{BASE_URL}/oslc/os/mxapiperuser",  # OSLC format
        f"{BASE_URL}/api/os/MXAPIPERUSER"   # Uppercase format
    ]
    ...
    query_params_options = [
        ...
        },
        {
            "lean": "0",  # Get full response
            "oslc.pageSize": str(limit),
            "oslc.select": "personid,userid,displayname,status,defaultsite,insertsite,primaryemail,primaryphone,department,supervisor,language,country,stateprovince,city,postalcode,spi:personid,spi:userid,spi:displayname,spi:status,spi:defaultsite,spi:insertsite,spi:primaryemail,spi:primaryphone",
            ...
            for headers in header_options:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"Successfully fetched peruser collection. Status code: {response.status_code}")
                        
                        # Save the raw response for debugging
                        with open('peruser_raw_response.json', 'w') as f:
    ...
    if not collection_data:
        logger.error("Failed to fetch peruser collection")
        return None
    ...
    if not found_member_key:
        ...
        # Check if we have typical peruser fields
        peruser_fields = ['personid', 'userid', 'displayname', 'status', 'spi:personid', 'spi:userid']
        has_peruser_fields = any(field in collection_data for field in peruser_fields)
        
        if has_peruser_fields:
            logger.info("Response appears to contain peruser data directly")
            # Wrap the data in a member array for consistent processing
            collection_data = {'member': [collection_data]}
            found_member_key = 'member'
        else:
            logger.error("Response does not contain peruser data")
            return None
    ...
    sample_user = collection_data['member'][0] if collection_data['member'] else {}
    field_count = len(sample_user.keys())
    ...
    if field_count <= 1:
        ...
        try:
            ...
            detailed_url = f"{BASE_URL}/api/os/mxapiperuser"
            detailed_params = {
                "oslc.select": "*",
                "oslc.pageSize": str(limit),
                "lean": "0",
    ...
    if collection_data['member']:
        ...
        logger.info(f"Peruser data has these fields: {', '.join(fields)}")
    
    logger.info(f"Using collection data with {len(collection_data['member'])} users")
...
def suggest_primary_key(data, field_summary):
    ...
    priority_fields = ['personid', 'userid', 'id', 'peruserid']
...
def suggest_sqlite_schema(field_summary, primary_keys):
    ...
    common_query_fields = ['status', 'defaultsite', 'insertsite', 'userid', 'displayname']
...
def main():
    """Main function to fetch and analyze peruser data."""
    # Fetch peruser data
    peruser_data = fetch_peruser_data(limit=50)  # Fetch up to 50 records for better analysis
    
    if not peruser_data:
        logger.error("Failed to fetch peruser data")
        return
    
    # Save raw data for reference
    with open('peruser_data.json', 'w') as f:
        json.dump(peruser_data, f, indent=2)
    
    logger.info(f"Fetched {len(peruser_data.get('member', []))} peruser records")
    
    # Analyze field types
    field_summary = analyze_field_types(peruser_data)
    ...
    potential_keys = suggest_primary_key(peruser_data, field_summary)
    
    # Print analysis results
    print("\n=== PERUSER ENDPOINT ANALYSIS ===\n")
    print(f"Total records analyzed: {len(peruser_data.get('member', []))}")
    ...
    # Save schema to file
    with open('peruser_schema.sql', 'w') as f:
        f.write(sqlite_schema)
    
    logger.info("Analysis complete. Results saved to peruser_data.json and peruser_schema.sql")
